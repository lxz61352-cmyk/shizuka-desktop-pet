...
- **聊天里说「做完了」**（`todo_reply._todo_done_from_chat`，桌面 `on_chat_submit` 与微信 `_weixin_reply` 都接了这条，本地判断不调模型）：`done_claim()` 先认出「已完成 / 已经检查了 / 已经喝了水了 / 搞定」这类说法并排掉否定与疑问（`还没做完`、`完成了吗`）。**中文会把宾语插在动词和「了」中间**，所以除了 `DONE_WORDS` 的字面短语，还有 `DONE_FORM_RE` 认「喝过水了 / 看完那本书了」这种带宾语的写法（`动词+(完|过|好|掉)+≤6 字宾语+了`）；另外 `WILL_DO_RE` 排掉「我要喝水了 / 准备把报告交了」这种打算做。再用 `match_score()` 把话对上未完成待办——标题核心词（`todo_core()` 去虚词）或原样标题被整段包含记 100 分，2 字以上连续片段记片段长度，「核心词按顺序基本都出现」（`subseq_cover()` ≥60%，所以「喝水」对「喝了水」「喝过水了」都能中）再加分；唯一最高分 → `_todo_done_apply()` 走 `_todo_complete_or_restore()` 收尾。对不上具体事项时按顺序退：刚提醒过且只剩一条 → 就是它；只剩一条待办且是「强说法」（`strong_claim()`，如「已完成」而「喝了」不算）→ 就是它；否则 `_todo_done_ask()` 列出候选并绑上 `choosing`（用户回「1」或标题由 `_todo_complete_reply` 接手）。**宁可问一句，也不乱划**。
- **踩过的坑**：`_apply_todo_action('complete')`（模型路由器那条路）原先只把 `done=True` 写进 `todos.json`，**没走** `_todo_complete_or_restore`——于是没取消未发出的提醒、周期待办也没推到下一次，用户会看到「说完成了却还在提醒」。现在它直接调 `_todo_complete_or_restore(todo, True)`，和待办窗口勾选是同一条路。另外模型路由器的闸门（`intent_routing.local_intent`）里原先没有「检查了」这类词，像「收到，已经检查了喵」会直接判成普通聊天、连路由器都不问——这也是当初没认出来的原因之一。
...
### 14. 联网查证（`src/web_search.py`，未发版）

**什么时候搜**（本地判定，不花时间也不联网）：`needs_search()` 按优先级判——明确要求（查一下/搜一下/联网查…）→ 日期 → 版本 → 数字 → 时事；`SKIP_RE` 先把算术、几点/今天几号、天气、待办、翻译、讲题、代码、写作排除掉。**别用"什么/为什么/是谁"这种疑问词当判据**：那些多半是模型本来就会的常识（实测「费米悖论是什么」不搜，1.7 秒答完）。

**搜索词要洗**：`cleanup_query()` 去掉客套、疑问词、时间词，并把「新发布/新出的」换成「最新」。这不是可选项——实测原样拿「最近有什么新发布的 AI 模型」去搜，引擎会按**那首叫《最近》的歌**返回结果。

**源链**（`default_sources()`，第一个拿到结果就返回）：

| 顺位 | 源 | 实测 |
|---|---|---|
| 1 | `cn.bing.com/search?format=rss` | **0.27–0.3 秒**、结构化 XML、**不需要加速器** —— 主源 |
| 2 | 360（`www.so.com/s`，`li.res-list` + `h3>a`） | 0.5–1.1 秒，直连可用 |
| 3 | 搜狗（`www.sogou.com/web`，`div.vrwrap` + `h3>a`） | 1.1 秒，直连可用 |
| 4 | `www.bing.com` RSS | 跟**出口地区**走（挂日本节点返回日文结果），所以排在国产源之后 |
| 5+ | DuckDuckGo / 维基 / Google News | **只有能直连外网才追加**（复用 `weather._net_mode()`）；不走代理时各卡满 10 秒 |

- **百度不能用**：返回的是 1.4KB 反爬验证页（`timeout hide-callback`），已排除。
- `TOTAL_BUDGET`=4.5 秒（整条链）、单源 3 秒；**主源返回空会重试一次**（实测 cn 偶发 0 条，同一批查询过一会儿又正常）；四个源的解析都是「块 + `h3>a` 标题 + 块内文字当摘要」。
- 缓存：`data/characters/<角色>/search-cache.json`，键是清洗后的搜索词，6 小时、最多 200 条；命中就直接回，不再出网。

**给模型的东西只有摘要**：`format_evidence()` 只放 3 条（标题 + 站点 + 140 字摘要），并写明「用两三句话概括结论，**不要照抄或逐条朗读**，别念网址，资料之间不一致就说明分歧，资料里没有的别编」；一条都没搜到就走 `format_failure()`，让她**直说没查到**再给一般性解释——跟「错了就不说」一致。

**接进对话**：`pet._web_evidence()` 在 `_ask_model()` 里把资料块拼进 system；`_announce_search()` 先来一句「让我查一下资料……」——有语音就进 TTS 队列（说话本身盖住搜索耗时），没语音就显示在加载气泡里；开关是 `settings.json` 的 `web_search`（菜单「更多设置 › 不确定的消息联网查证」），`sound.log` 里有 `search:` 行记录用了哪个源、几条、耗时、是否命中缓存。

**还没做**：模型自动改写搜索词（现在纯本地清洗）；多轮追问的指代（「它呢」）；来源做成可点链接；`read_web_page()` 抓正文已写好但还没接到「让她读一整页」的入口。
...
**联网查证**：新增 `src/web_search.py` + `pet._web_evidence()`，明确要求 / 时事 / 版本 / 数字 / 日期才联网，搜索词先本地清洗，主源是**不需要加速器**的 `cn.bing.com` RSS（0.27 秒），外网源只在能直连时追加；给模型的只有 3 条摘要并要求概括不照读，搜不到就让她直说。详见「四、14」。
...
## 四、14 补充：联网查证现行实现（2026-09 更新）

上文几处已经过时了（"只把摘要交给模型""第一个成功的源就返回"），现行实现是：

- **搜索词**：`cleanup_query()` 洗完 → `search_query()` 按意图补词（`发布`→`发布时间`、`版本`→`最新版本`、
  `股票`→`股价`）。实测同句差别巨大：`iPhone 18 发布` 是官网换购页，`iPhone 18 发布时间` 才是官方发布稿。
  **不要自动加年份**，加了会把结果带偏（踩过）。
- **取结果**：多源**并发**（`bing-rss-cn` + `bing-html-cn` + `360` + `搜狗`，外网源仅在能直连时追加），
  第一个出结果的再等 `GRACE_AFTER_FIRST`（0.9s）等别的源，合并去重后按
  "关键词覆盖 + 站点可信度 + 多源印证 + 引擎顺序 + 新鲜度"打分（`score_result`）。
- **取内容**：对最靠前的 `PAGE_FETCH` 条**抓正文**（`read_web_page` → `extract_text`，丢脚本/导航/页脚、
  优先 `<article>/<main>`），正文优先于摘要；顺带修了 GBK 页面按 UTF-8 解码成乱码的问题（`_decode`）。
- **不对题就不要**：`on_topic()` 用标题命中搜索词片段的数量做闸门（短问句门槛自动降为 1），
  一条不剩就当没搜到，让 `format_failure` 教她直说没查到——宁可说不知道，也别把新闻列表首页当资料。
- **缓存**：时事/版本/数字/日期类 TTL 45 分钟（`cache_ttl_for`），其余 6 小时。
- **语音**：合成前过 `speakable()`，只念中文和英文（日文假名会让合成卡住并拖坏后面整段语音）。
