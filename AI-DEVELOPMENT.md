...
**接进对话**：`pet._web_evidence()` 在 `_ask_model()` 里把资料块拼进 system；`_announce_search()` 先来一句「让我查一下资料……」——有语音就进 TTS 队列（说话本身盖住搜索耗时），没语音就显示在加载气泡里；开关是 `settings.json` 的 `web_search`（菜单「更多设置 › 联网查证」），`sound.log` 里有 `search:` 行记录用了哪个源、几条、耗时、是否命中缓存。
