...
NOT_DONE_RE = re.compile(r'(?:还没|未|没有|没|不)(?:有)?(?:做|弄|搞|看|读|写|背|检查|交|买|喝|吃|完成|处理)'
                         r'|不用|不必|不要|取消|忘了|忘记|回头再|等会儿|一会儿|打算|准备|要不要|能不能|是不是')
...
def done_claim(text):
    ...
    if len(t) <= 10 and re.search(r'(?:吗|么|吧)$', t):   # 「完成了吗」「搞定了吧」是在问，不是在报
        return None
    if not any(word in t for word in DONE_WORDS):
        return None
    return t
