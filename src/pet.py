...
class DeskPet:
    ...
    def _comment_foreground(self, title, exe):
        ...
        prompt = (
            ...
            "以静香的口吻，就他正在做的事说一句简短自然的话（一到两句，口语化），可以关心、调侃、感慨或提醒。\n"
            "要求：\n"
            "1) 开头要有变化，**不要用「又在……」开头**，也不要套「又在 + 做什么」这个固定句式；\n"
            "2) 尽量结合这个程序/窗口的具体内容说，别泛泛而谈，也别生硬地念程序名；\n"
            "3) 不要像系统通知，不要复述「你在用 xxx」；\n"
            "4) 前面的对话只作参考，别照搬句式。\n"
            "参考这几种不同开头的感觉（别照抄）：\n"
            "「这游戏一玩就停不下来吧？脖子先活动两下。」\n"
            "「代码写得顺不顺？卡住了就起来倒杯水。」\n"
            "「说好只看一个视频，结果又刷上了。」"
        ) % (now_str, title or exe, exe)
        try:
            client = get_client()
            resp = client.chat.completions.create(
                model=api_model(),
