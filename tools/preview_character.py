...
def main():
    ...
    root=tk.Tk()
    root.title("Shizuka 0.5.6 · 拖动抚摸与双跳")
    ...
    ttk.Label(panel,text="侧面局部动作",font=("Microsoft YaHei UI",16,"bold")).pack(anchor="w",pady=(0,14))
    ...
    controller=MotionController();ground=GroundMotion()
    ...
    tk.Label(panel,text="提起撇嘴 → 下落轻微惊讶 → 落地恢复。",bg="#f3f5fa",fg="#526071").pack(anchor="w",pady=(14,8))
    ...
    def move(event):
        ...
        if not controller.dragging and max(abs(dx),abs(dy))>4:
            controller.begin_drag((press[0]/height,press[1]/height),
                ((press[2]-press[4])/(renderer.canvas_size[0]*height/renderer.canvas_size[1]),(press[3]-press[5])/height),
                time.monotonic())
        if controller.dragging:
            offset[:]=[press[4]+dx,max(-15,min(floor_top(),press[5]+dy))]
    ...
    def region(event):
        ...
        box=selected.manifest.get("interaction_regions",{}).get("head_pat",
